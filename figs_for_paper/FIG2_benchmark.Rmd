...
```{r}
am@tag = factor(am@tag,levels=c('uniform','2-lattice','best_disc_free'))

colors=c('best_disc_free'        = '#F57600',
         '2-lattice'        = '#0073e6',
         'uniform'          = 'black')
...
tag   = c('uniform','2-lattice','best_disc_free')
...
start=Sys.time()
amps  = seq(from=1,to=3,length.out=2^4)
...
load_all()
start=Sys.time()
tags = c('uniform','2-lattice','best_disc_free')
fam=NULL
for (tag in tags){
  if (tag=='uniform'){
    mt_loc=mt_unif
