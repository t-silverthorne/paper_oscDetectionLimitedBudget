...
```{r}
df_tot=df_tot_bkup
df_tot$feature =factor(df_tot$feature,levels=c('rao','fder'),labels = c('non-uniformity','frequency sensitivity')) 
df_tot$type=factor(df_tot$type,levels=c('random','optimal'),labels = c('random','near optimal')) 
df_tot$Nmeas =factor(df_tot$Nmeas,levels=c(12,24,48),labels = c('12','24','48')) 

p1=df_tot %>% ggplot(aes(x=ncp,y=y,color=type))+geom_point(size=1)+facet_grid(feature~Nmeas,scales='free')
...
p1c = p1 + theme(
  ...
  x='non-centrality',
  y=element_blank(),
  color='measurement distribution'
)&theme(legend.position='bottom',plot.margin=margin(0,0,0,0))
p1c

ggsave(paste0(overleaf_plot_directory,'multiobjective.png'),p1c,width = 6,height=4,device='png',dpi=600)
