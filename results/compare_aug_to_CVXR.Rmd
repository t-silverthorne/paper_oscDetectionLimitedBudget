...
p1=mtdf_full %>% ggplot(aes(x=time,y=ncp,color=solver))+geom_point(size=.7)+
...
if (quick_test){
  ...
  pers  = seq(from=1/24,to=1,length.out=2^2)
}else{
  amps  = seq(from=.5,3.0,by=.05)
  pers = 10^seq(from=log10(1/24),to=log10(1),length.out=1e2)
}
...
if (quick_test){
  ...
  NacroFDR=2^2
  amps  = seq(from=.5,3.0,by=.05)
  pers = 10^seq(from=log10(1/24),to=log10(1),length.out=5e2)
}else{
  Nmc=5e2
  NacroFDR=2^5
  NfreqFDR=2^5
  amps  = seq(from=.5,3.0,by=.1)
  pers = 10^seq(from=log10(1/24),to=log10(1),length.out=2^6)
}
tag   = c('uniform','pin2lat','cvxr')
...
ggsave(paste0(overleaf_plot_directory,'cvxrVaug_powerFDR.pdf'),F1,width = 6,height=6)
F1
```