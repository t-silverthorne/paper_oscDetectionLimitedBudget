...
ggsave(paste0('~/research/ms_powerCHORD/figures/','motivation_fig_v2.png'),Fig,width=plt_width,height=plt_height,device='png',dpi=600)
...
PLT = psol/pcurve/phmap + plot_annotation(tag_levels='A')+
  plot_layout(heights=c(2,1,1))&theme(text=element_text(size=9))
...
PLT = plt_raw/plt_mhmap/plt_conv  + plot_annotation(tag_levels='A')+
  plot_layout(heights = c(.7,1.5,1))
...
plt = fdf %>% ggplot(aes(x=fmin,y=fmax,fill=d_power))+geom_tile(height=2)+facet_wrap(~Nmeas,nrow=1)+scale_fill_viridis_c(limits=c(0,1)) 
...
## Compare adding a point to redistributing 
```{r}
