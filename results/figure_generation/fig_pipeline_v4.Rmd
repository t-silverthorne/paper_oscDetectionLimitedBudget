...
```{r}
# redo TPR estimates
dfTPR = readRDS('results/data/MC_periodogram.RDS')

dfTPR$f = factor(dfTPR$freq,unique(dfTPR$freq),paste0('f = ',unique(dfTPR$freq)))
p1=dfTPR %>% filter(p_osc==.1 & fdr_method =='BH') %>% ggplot(aes(x=alpha_val,y=TPR,group=type,color=type))+
  geom_line()+
  facet_grid(p_method~f)#Amp+p_method

p2=dfTPR %>% filter(p_osc==.5 & fdr_method =='BH') %>% ggplot(aes(x=alpha_val,y=TPR,group=type,color=type))+
  geom_line()+
  facet_grid(p_method~f)#Amp+p_method
```
...
p1 = p1 + scale_x_continuous(limits =c(xmin,xmax),breaks=c(0,.25),labels=c(0,0.25)) +
...
p2 = p2 + scale_x_continuous(limits =c(xmin,xmax),breaks=c(0,.25),labels=c(0,0.25)) +
  scale_y_continuous(limits=c(0,1))
p2 = p2 + theme(
  strip.background=element_blank(),
  plot.margin = margin(0,0,0,0),
