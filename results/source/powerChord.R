...
powerChord=function(Nmeas,fmin=1,fmax=24,
  ...
  zero_vec        = rep(0,Nfine+2)
  rt_inds         = seq(1,Nfine,drts)
  lin_csts        = list()
  rhs_list        = list()
  sense_list      = list()
  lin_csts[[1]]   = c(rep(1,Nfine),0,0)
  rhs_list[[1]]   = Nmeas
  sense_list[[1]] = '=' 
  
  # if support constraints are active
  if (drts<Inf){
    lin_cstr_vec          = rep(0,Nfine+2)
    rt_inds               = seq(1,Nfine,drts)
  ...
  for (ii in c(1:Nfreq)){
    ...
    qcmat  = cbind(rbind(qcmat,rep(0,Nfine),rep(0,Nfine)),rep(0,Nfine+2))
    qc     = c(rep(0,Nfine),-1,w_reg)
    beta   = 0
    sense  = '<'
  ...
  if (w_reg>0){
    # TODO: check this matches MATLAB expression 
    ...
    qcmat_reg                  = cbind(rbind(ncp_reg_mat,rep(0,Nfine),rep(0,Nfine)),rep(0,Nfine+2))
    qcmat_reg[Nfine+2,Nfine+2] = -1
    qc_reg                     = rep(0,Nfine+2) # TODO: see if this can be left unspecified
    beta_reg                   = 0 
    sense_reg                  = '<'
    model$quadcon[[Nfreq+1]] = list(Qc=qcmat_reg,q=qc_reg,rhs=beta_reg,sense=sense_reg)
  }
