...
```{r}
load_all()
freqs_global  = seq(from=1,to=24,length.out=2^8)
Nsh=2^8
...
for (sc1 in scvals){
  for (Nmeas in Nmeas_vals){
    ...
    cfvals = c(1:length(shvals)) %>% as.list() %>% mclapply(mc.cores=mc_cores,function(ii){
      costfun_2lattice_svdpower(0,shvals[ii],scale1,scale2,lat1,lat2,freqs=freqs_global,
                              cfuntype='ncp',gapPenalty=Nmeas)}) %>% unlist()
...
#cfun = function(x){-costfun_2lattice_svdpower(0,x,scale1,scale2,lat1,lat2,
#                                             freqs=freqs_global,cfuntype='ncp',
#                                             regL1=0,gapPenalty=0,regFder=0) }
#xopt=stats::optim(par=runif(1),fn=cfun,method='SANN',
#                  control=list(maxit=40,REPORT=1,trace=6))
...
xopt=wrapper_sweep_lattice(opt_osc_power,
                                Nvals    = c(8,16),
                                Nmeas    = gset$Nmeas,
                                cts_flag = T,
                                ...
                                cfuntype= 'ncp')
xopt
df_full %>% ggplot(aes(x=x,y=y,group=reg,color=reg))+geom_line()+
  geom_point(data=data.frame(x=xopt$res_best$optim_raw$par[1],y=-xopt$res_best$optim_raw$value,reg='opt'),aes(x=x,y=y))

ggplot()+geom_point(aes(x=xopt$res_best$mtvalue,y=1))
```


## Can SANN discrete find optimum 
```{r}
