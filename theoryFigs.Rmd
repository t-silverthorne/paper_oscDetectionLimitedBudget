...
PDES = df_PDES %>% ggplot(aes(x=time,y=sched,color=sched))+
  geom_point() + scale_color_manual(values=vis_colors)+theme(legend.position = "none")+facet_wrap(~Nmeas,nrow=length(Nmeasvec),strip.position = 'top')
```
...
PDET = df_PDET %>% ggplot(aes(x=freq,y=detRedFIM,group=sched,color=sched))+geom_line()+facet_wrap(~Nmeas,nrow=length(Nmeasvec))
```
...
```{r}
{PDES | PDET | PPWR}+plot_annotation(tag_levels='A')+plot_layout(guides='collect') &theme(legend.position = 'bottom',
        legend.direction = 'horizontal',
        legend.box = 'horizontal')
ggsave(paste0(overleaf_plot_directory,"fig_reduced_design_space.pdf"), width = 7, height =6)

```
## Plot phase/freq scatter plot for each design
```{r}
```
