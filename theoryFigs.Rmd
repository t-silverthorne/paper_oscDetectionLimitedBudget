...
PDES = df_PDES %>% ggplot(aes(x=time,y=sched,color=sched))+
  geom_point()+theme(legend.position = "none")+facet_wrap(~Nmeas,nrow=length(Nmeasvec),strip.position = 'top')
```
...
PDET = df_PDET %>% ggplot(aes(x=freq,y=detRedFIM,group=sched,color=sched))+geom_line()+facet_wrap(~Nmeas,nrow=length(Nmeasvec),scales='free_y')
```
...
PPWR = df_PPWR %>% ggplot(aes(x=freq,y=pwr,group=amp,color=amp))+geom_line()+facet_grid(Nmeasloc~sc_type,switch = 'y')+scale_color_viridis_c()
```


```{r}
PDES2 = PDES+guides(color=F)
{PDES2  | PDET | PPWR}+plot_annotation(tag_levels='A')+plot_layout(widths = c(1,1,2),guides='collect') &theme(legend.position = 'bottom',
        legend.direction = 'horizontal',
        legend.box = 'horizontal')
ggsave(paste0(overleaf_plot_directory,"fig_reduced_design_space.pdf"), width = 7, height =5.5)
...
simdat[,3:(length(tvec)+2)] %>% write.csv('temp.csv',row.names = T)
...
mout = meta2d('temp.csv',filestyle = 'csv',
       ...
       outputFile = F,adjustPhase = 'notAdjusted') 

pdf = data.frame(freq=simdat[,1],acro=simdat[,2],
           freq_est = 1/mout$LS$Period,
           acro_est =mout$meta$LS_adjphase) 

pdf %>% ggplot(aes(x=freq,y=freq_est))+geom_point()
pdf %>% ggplot(aes(x=acro,y=acro_est))+geom_point()
