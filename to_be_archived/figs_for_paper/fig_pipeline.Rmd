...
F1_slide = ((F1raw[[1]]+labs(title='A'))+F1raw[[2]] +
              (F2raw[[1]]+labs(title='B')) + F2raw[[2]])+
plot_layout(guides='collect',nrow=1)&
  theme(legend.position='bottom',
     ...
     panel.grid.minor = element_blank(),
     axis.text.x = element_text(vjust = 0.25)
  )& labs(color='period') #&scale_y_continuous(n.breaks=3)
F1_slide
ggsave(paste0(slide_version_directory,'motivation_fig_v2.png'),F1_slide,width=10,height=6,device='png',dpi=600)
```
...
fs_slide=15
F1_slide =as.ggplot(F1raw&theme(legend.position = 'none',text=element_text(size=fs_slide)))
F2_slide =as.ggplot(F2raw&theme(legend.position = 'bottom',text=element_text(size=fs_slide)))

ggsave(paste0(slide_version_directory,'motivation_fig_v2_unif.png'),F1_slide,width=8,height=6,device='png',dpi=600)
ggsave(paste0(slide_version_directory,'motivation_fig_v2_alt.png'),F2_slide,width=8,height=6,device='png',dpi=600)
```
```{r}
...
Fig_slide = F1raw+F2raw + plot_annotation(tag_levels ='A')+plot_layout(guides='collect')&
  theme(legend.position = 'bottom',text=element_text(size=fs_slide))
Fig_slide
ggsave(paste0(slide_version_directory,'motivation_fig_v2.png'),Fig_slide,width=8,height=6,device='png',dpi=600)
```
...
flist
sol_factor_levels = c('uniform','2hr','3hr',
                                 'free')
```
...
mtdf %>% head()
mtdf[grepl('sol_gur_free',mtdf$solver),]$solver='free'
#mtdf[grepl('drts_6',mtdf$solver),]$solver='1hr'
mtdf[grepl('drts_12',mtdf$solver),]$solver='2hr'
mtdf[grepl('drts_18',mtdf$solver),]$solver='3hr'
```
...
```
```{r}
fig=(p1+labs(y='measurement method')+theme(legend.position='none'))/p3 + plot_layout(heights=c(1,2),guides='collect') & theme(
     strip.background=element_blank(),
     text=element_text(size=14),
     axis.title.x=element_text(size=12),
     axis.title.y=element_text(size=12),
...
ggsave(paste0(slide_version_directory,'numerical_res.png'),fig,width=10,height=6,device='png',dpi=600)
```
```{r}
p1c = p1+theme(legend.position='none')
p3c = p3 +theme(
     legend.position = 'bottom',
     legend.box = 'vertical')
Fig_slide=p1c/p3c + plot_layout(heights = c(.3,1.5)) + plot_annotation(tag_levels='A')  &
   theme(
     legend.position='bottom',
     strip.background=element_blank(),
     text=element_text(size=fs_slide),
     plot.margin=margin(0,0,0,0),
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.text.x = element_text(vjust = 0.25)
  ) 
ggsave(paste0(slide_version_directory,'cvxrVaug_powerFDR.png'),Fig_slide,width=8,height=6,device='png',dpi=600)
...
mdf=names %>% lapply(function(solver){
  ...
  ests = c(1:Nmc) %>% mclapply(mc.cores=mc_cores,function(jj){
    ...
    fits$qval = p.adjust(fits$pvalue,method='fdr')
    return(fits[which.min(fits$qval),])
  })
...
ggsave(paste0(overleaf_plot_directory,'accuracy_benchmark.png'),Fig,width = 6,height=6,device='png',dpi=300)
