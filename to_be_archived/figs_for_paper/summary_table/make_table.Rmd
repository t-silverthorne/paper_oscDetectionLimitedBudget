...
pdf[pdf$solver== 'supp_1hr',]$solver = '1hr'
pdf[pdf$solver== 'supp_2hr',]$solver = '2hr'
pdf[pdf$solver== 'supp_3hr',]$solver = '3hr'

pdf$solver = fct_expand(factor(pdf$solver),'uniform')
Fig=pdf[pdf$wrap=='gurobi_raw',] %>% ggplot(aes(x=Nmeas,y=ncp,group=solver,color=solver))+
  geom_point(aes(shape=stat_code),size=3)+
  theme(
...
)+labs(x='measurement budget',
       y='non-centrality parameter',
       color='measurement \nmethod',
       shape='result',
       )
ggsave(paste0(overleaf_plot_directory,'optimality_compare.png'),Fig,width = 10,height=6,device='png',dpi=600)
